<template>
  <div class="scm-attachment-card">
    <div v-if="fileLimit">附件上限：{{fileLimit}}</div>
    <div class="clearBoth isDialogCallWrap" slot="header" v-if="!(hideTitle && readonly)">
      <span v-if="!(hideTitle || isDialogCall)"
        :class="[ required ? 'is-required' : '']">{{ title }}</span>
      <scm-upload class="attachment-card__attachment-file-upload"
        v-if="!innerBusinessTypeConfig || innerBusinessTypeConfig.length <= 0"
        :class="[ isDialogCall ? 'isDialogCall' : '']"
        :disabled="judgeLimit"
        ...
        <span class="attachment-menu el-dropdown-link"
...
export default {
  ...
  computed: {
    ...
    attachTableData() {
      ...
      return this.addFileList.concat(propData);
    },
    fileUploadUrl() {
  ...
  methods: {
    ...
    onControlButtonClick(iconName) {
      if (iconName === 'download') {
        ...
        // this.$store.dispatch('attachment/downloadFiles', idArray);
    ...
    beforeFileUpload(file) {
      ...
      if (!this.beforeFileUploadFun(file)) {
        return false;
      }
      if (file && file.size === 0) {
    ...
    onFileChange(file, fileList) {
      ...
      this.$emit('attachment-change', emitList);
    },
    /**
     * 文件上传时的钩子
     */
    ...
    emitAddFileChange(currentFile) {
      ...
      this.$emit('attachment-add', currentFile, ret, [...this.data, ...this.addFileList]);
    },
    /**
     * 文件上传失败的钩子
     * @event
    ...
    deleteFile(fileInfo) {
      ...
      this.$emit('attachment-delete', fileInfo, this.deleteFileList, [...this.data, ...this.addFileList]);
    },
