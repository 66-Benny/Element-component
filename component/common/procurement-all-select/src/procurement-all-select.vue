  ...
  <basic-dialog :visible.sync="innerVisible"
    ...
    <div class="scm-procurement-select">
      ...
      <scm-form :model="condition"
        ...
        <scm-form-item label="创建日期（止）"
          ...
          <scm-form-item label="创建人"
                         prop="prjMgr">
            <scm-input v-model="condition.prjMgr"
                       class="procurement-name"></scm-input>
          </scm-form-item>
        </div>
      </scm-form>
      ...
      <scm-table :data="procurement.viList"
        ...
        <scm-table-column label="控制方式"
                          prop="numAmount"
                          :formatter="numAmountFormatter"
...
const FETCH_PROCUREMENT_ACTION = 'mdm/fetchProcurementAll';

export default {
  name: 'ScmProcurementAllSelect',
  ...
  props: {
    ...
    // },
    purType: {
      // 默认初始化查询 ''、全部;-1、全部一采;-2、全部二采;1、一级集采;2、总部自采-横向部门;3、二级集采;
      // 4、省自采-横向部门;5、总部来函一采项目;6、合同项目;7、分公司采购;8、分配一采结果项目
      type: String,
      default: '1',
    },
    createDepID: {
  ...
  computed: {
    ...StoreGlobalAPI.mapState({
      ...
      procurement: state => state.mdm.procurementAll,
    }),
    clearDisabled() {
  ...
  methods: {
    ...
    numAmountFormatter(row, column, cellValue) {
      if (cellValue === '1') {
        return '数量';
      } else if (cellValue === '0') {
        return '金额';
